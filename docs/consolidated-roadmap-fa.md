...
- مرحلهٔ ۵BX: با `DEC-263` پذیرفته و بسته شده است. فقط audit package پذیرفته‌شدهٔ evidence Golden محلی با JCS/SHA-256 digest می‌شود؛ package ناموجود/ردشده digest ندارد و persistence، انتقال، کیفیت داده/label، candidate، Paper و اجرا خارج از دامنه‌اند.
- مرحلهٔ ۵BY: با `DEC-265` پذیرفته و بسته شده است. فقط digest audit package محلی دوباره محاسبه و با هش موردانتظار تطبیق می‌شود؛ ورودی نامعتبر، عدم‌تطبیق یا package ناموجود fail-closed است و persistence، انتقال، کیفیت داده/label، candidate، Paper و اجرا خارج از دامنه‌اند.
- مرحلهٔ ۵BZ: با `DEC-266` در حال اجرا است. فقط ناوبری presentation-only PWA با fragmentهای محلی، بازخورد اعلان و رفتار back/forward مرورگر تعمیر می‌شود؛ pairing، cloud sync، داده/اتصال معاملاتی، Paper و اجرا خارج از دامنه‌اند.
- مرحلهٔ فعال: ندارد؛ Paper entry، `OrderIntent`، fill، position، P&L، persistence، UI، AI signal و هر اتصال همچنان گیت مستقل و تأیید صریح مالک می‌خواهند.
- مرحلهٔ فعال: ندارد؛ Paper entry، `OrderIntent`، fill، position، P&L، persistence، UI، AI signal و هر اتصال همچنان گیت مستقل و تأیید صریح مالک می‌خواهند.
